...
/// soft-deleted by a flag. Cloud Firestore is a document store, so each entity becomes
/// one document keyed by its stable device id, last-write-wins still falls out of
/// comparing `updatedAt`, and a soft delete is just a document with its flag set.
///
/// ## Why the REST surface rather than the native SDK
///
/// The terminal reaches Firestore and Firebase Authentication over their HTTPS APIs
/// through a thin `dart:io` client, exactly as the previous backend did. That keeps the
/// desktop build free of native plugins and CocoaPods, keeps the dependency list
/// minimal, and lets the whole sync path be tested without a live project. The data and
/// the security rules are identical to what a native-SDK client would see, so an Android
/// app built on FlutterFire shares the same Firestore and the same accounts.
///
/// ## What is safe to hold here
///
/// Only client-safe values. The Firebase project id and the Web API key are designed to
/// ship in client applications; access is controlled by Firestore Security Rules on the
/// server, not by keeping these secret. The optional [refreshToken] is a signed-in
/// user's session, exchanged for short-lived ID tokens; it is not a password, and the
/// privileged service-account key is never placed in the application, in configuration
/// or in source.
class FirebaseConfig {
  ...
  /// The host every Firestore request resolves, used by the connectivity probe.
  static const String firestoreHost = 'firestore.googleapis.com';

  /// The Firebase project id.
  final String projectId;

  /// The Web API key. Client-safe by design; Firestore Security Rules are what
  ...
  ///
  /// The refresh token is part of "configured" on purpose. Firestore Security Rules
  /// deny an anonymous client everything, so a terminal with no session has no backend
  /// it can actually reach; treating that as "not configured" keeps it cleanly offline
  /// rather than failing every request against the rules.
  ...
  String? get host => isConfigured ? firestoreHost : null;
}
